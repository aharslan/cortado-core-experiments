...
ALGORITHMS_DIJKSTRA = {
    ALGORITHM_TP_DIJKSTRA_ADVANCED,
    ALGORITHM_BASELINE_DIJKSTRA_NOT_NAIVE,
    ALGORITHM_BASELINE_DIJKSTRA,
}
ALGORITHMS_A_STAR = {
    ALGORITHM_TP_A_STAR_ADVANCED,
    ALGORITHM_BASELINE_A_STAR_NOT_NAIVE,
...
plot_configs = [
    ("DIJKSTRA", ALGORITHMS_DIJKSTRA, DIJKSTRA_RENAMINGS, 3),
...
palette = {
    ...
    BASELINE_NOT_NAIVE_LABEL: sns_palette[1],
    ADVANCED_LABEL: sns_palette[2],
}
...
def __create_plots(
    filename: str, directory: str, infix_type: str, results_df: pd.DataFrame
):
    for name, algorithms, renaming, n_cols in plot_configs:
        Path(os.path.join(directory, "plots", filename, infix_type, name)).mkdir(
        ...
        for with_outliers in [True, False]:
            ...
            for attribute, y_label, should_save in attributes_with_title:
                ...
                d_cpy = d.copy()
                if (
                    attribute == "Preprocessing Duration"
                    or attribute == "Added Tau Transitions"
                ):
                    d_cpy = d_cpy[d_cpy["Algorithm"] != BASELINE_LABEL]
                    baseline_mean = d[d["Algorithm"] == BASELINE_LABEL][
                        attribute
                    ].median()
                ...
                sns.boxplot(
                    ...
                    hue_order=[
                        BASELINE_LABEL,
                        BASELINE_NOT_NAIVE_LABEL,
                        ADVANCED_LABEL,
                    ],
                )
                handles, labels = plt.gca().get_legend_handles_labels()
                order = [
                    labels.index(BASELINE_LABEL),
                    labels.index(BASELINE_NOT_NAIVE_LABEL),
...
def create_plots(filename: str, directory: str, infix_type: str):
    ...
    results_df = pd.read_csv(
        os.path.join(directory, filename + f"_{infix_type}_results.csv")
    )
