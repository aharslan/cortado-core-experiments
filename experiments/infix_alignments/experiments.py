...
from experiments.infix_alignments import create_plots
...
DATA_PATH = os.getenv("DATA_PATH", "C:\\sources\\arbeit\\cortado\\event-logs\\")
DATA_FILENAME = os.getenv("DATA_FILENAME", "PrepaidTravelCost")
...
algorithm_variants_infix = {
    ALGORITHM_BASELINE_DIJKSTRA: lambda trace, tree: infix_alignments.calculate_optimal_infix_alignment(
        trace,
        tree,
        use_dijkstra=True,
        naive=True,
    ...
    ALGORITHM_BASELINE_A_STAR: lambda trace, tree: infix_alignments.calculate_optimal_infix_alignment(
        trace,
        tree,
        use_dijkstra=False,
        naive=True,
    ...
    ALGORITHM_BASELINE_DIJKSTRA_NOT_NAIVE: lambda trace, tree: infix_alignments.calculate_optimal_infix_alignment(
        trace,
        tree,
        use_dijkstra=True,
        naive=False,
    ...
    ALGORITHM_BASELINE_A_STAR_NOT_NAIVE: lambda trace, tree: infix_alignments.calculate_optimal_infix_alignment(
        trace,
        tree,
        use_dijkstra=False,
        timeout=TIMEOUT,
    ...
    ALGORITHM_INFIX_ALIGNMENTS_DIJKSTRA_NOT_NAIVE: lambda trace, tree: infix_alignments.calculate_optimal_infix_alignment(
        trace,
        tree,
        naive=False,
        use_dijkstra=True,
...
algorithm_variants_postfix = {
    ...
    ALGORITHM_BASELINE_A_STAR_NOT_NAIVE: lambda trace, tree: suffix_alignments.calculate_optimal_suffix_alignment(
        trace,
        tree,
        naive=False,
        use_dijkstra=False,
    ...
    ALGORITHM_BASELINE_DIJKSTRA: lambda trace, tree: suffix_alignments.calculate_optimal_suffix_alignment(
        trace,
        tree,
        use_dijkstra=True,
        naive=True,
    ...
    ALGORITHM_BASELINE_A_STAR: lambda trace, tree: suffix_alignments.calculate_optimal_suffix_alignment(
        trace,
        tree,
        use_dijkstra=False,
        naive=True,
    ...
    ALGORITHM_INFIX_ALIGNMENTS_DIJKSTRA_NOT_NAIVE: lambda trace, tree: suffix_alignments.calculate_optimal_suffix_alignment(
        trace,
        tree,
        naive=False,
        use_dijkstra=True,
        timeout=TIMEOUT,
        variant=SUFFIX_TREE_BASED_APPROACH,
    ),
    ALGORITHM_INFIX_ALIGNMENTS_A_STAR_NOT_NAIVE: lambda trace, tree: suffix_alignments.calculate_optimal_suffix_alignment(
        trace,
        tree,
        naive=False,
        use_dijkstra=False,
...
def run_infix_alignment_experiments_for_file(log_filename_prefix):
    ...
    create_plots.create_plots(
        os.path.basename(log_filename_prefix),
        os.path.dirname(log_filename_prefix),
        INFIX_TYPE,
    )
...
def get_infixes(variants, log_filename_prefix):
    infixes_filename = (
        log_filename_prefix + "_infixes_" + str(RANDOM_SAMPLE_SIZE) + ".pkl"
    )
...
def __print_parameters(log_filename_prefix: str):
    print(
        ...
        os.path.exists(log_filename_prefix + PROCESS_TREE_EXTENSION),
    )
...
def __run_experiments_for_infix(infix, tree: ProcessTree):
    try:
        ...
        algorithm_variants = (
            algorithm_variants_infix
            if INFIX_TYPE == INFIX_TYPE_INFIX
            else algorithm_variants_postfix
        )

        for algorithm, fn in algorithm_variants.items():
            alignment = fn(infix_trace, tree)
            result.append(
                __build_measurement_data_for_alignment(
                    infix, infix_length, alignment, algorithm
                )
            )
...
def __build_measurement_data_for_alignment(infix, infix_length, alignment, algorithm):
    ...
    if (
        algorithm == ALGORITHM_BASELINE_A_STAR
        or algorithm == ALGORITHM_BASELINE_DIJKSTRA
    ):
        ...
        consumed_time = (
            alignment["alignment_duration"] + alignment["preprocessing_duration"]
        )
    ...
    return [
        ...
        algorithm,
        consumed_time,
        alignment["visited_states"],
        alignment["queued_states"],
        alignment["cost"],
        alignment["preprocessing_duration"],
        alignment["alignment_duration"],
        False,
        alignment["added_tau_transitions"],
        alignment["alignment"],
        alignment["lp_solved"],
    ]
